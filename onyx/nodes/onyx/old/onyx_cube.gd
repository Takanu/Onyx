...
# Used to determine how much the interior faces move away from the
# edge of the current box.  Not publically accessible 
...
#	print("[OnyxCube] ", self.get_name(), " - _get_property_list()")
	...
	# Quickly generate hollow margins
	var margin_name = ["x_minus", "x_plus", "y_minus", "y_plus", "z_minus", "z_plus"]
	for margin in margin_name:
		props.append({
...
#			print("Same value assignment, BAIIIII")
	...
	# ///// HOLLOW MARGINS /////
	if property.find("hollow_mode/") != -1:
		match property:
			"hollow_mode/x_minus_margin":
				_x_minus_hollow = value
		
			"hollow_mode/x_plus_margin":
				_x_plus_hollow = value
			
			"hollow_mode/y_minus_margin":
				_y_minus_hollow = value
			
			"hollow_mode/y_plus_margin":
				_y_plus_hollow = value
			
			"hollow_mode/z_minus_margin":
				_z_minus_hollow = value
			
			"hollow_mode/z_plus_margin":
				_z_plus_hollow = value
		
		if property.find("margin") != -1:
...
#	print("[OnyxCube] ", self.get_name(), " - _get() : ", property)
	...
	if property.find("hollow_mode/") != -1:
		match property:
			"hollow_mode/x_minus_margin":
				return _x_minus_hollow
		
			"hollow_mode/x_plus_margin":
				return _x_plus_hollow
			
			"hollow_mode/y_minus_margin":
				return _y_minus_hollow
			
			"hollow_mode/y_plus_margin":
				return _y_plus_hollow
			
			"hollow_mode/z_minus_margin":
				return _z_minus_hollow
			
			"hollow_mode/z_plus_margin":
				return _z_plus_hollow
...
# Using the set handle points, geometry is generated and drawn.  
# The handles owned by the gizmo are also updated.
...
#	# Prevents geometry generation if the node hasn't loaded yet
	...
	onyx_mesh = build_geometry(x_minus_position, x_plus_position, y_minus_position,
...
# Using the set handle points, geometry is generated and drawn.  
func update_hollow_geometry():
	
	# Prevents geometry generation if the node hasn't loaded yet
	if is_inside_tree() == false || Engine.editor_hint == false || hollow_object == null:
		return
	
	print("[OnyxCube] ", self.get_name(), " - update_hollow_geometry()")
	...
	var new_onyx_mesh = build_geometry(x_minus_diff, x_plus_diff, y_minus_diff,
			y_plus_diff, z_minus_diff, z_plus_diff)
	
	_update_hollow_mesh(new_onyx_mesh)
...
func build_geometry(x_minus : float,  x_plus : float,  y_minus : float,  y_plus : float, 
	...
	print("[OnyxCube] ", self.get_name(), " - build_geometry()")
