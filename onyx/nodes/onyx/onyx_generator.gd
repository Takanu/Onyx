...
#	print("[OnyxGenerator] ", self, " - get_control_data()")
	...
	var result = {}
	for control in acv_controls.values():
...
#	print("[OnyxGenerator] ", self, " - get_control_data()")
	...
	for data_key in data.keys():
		acv_controls[data_key].set_control_data(data[data_key])
...
func get_gizmo_undo_state(control_point):
	var saved_translation = self.global_transform.origin
	return [pre_controls.duplicate(false), saved_translation]
...
#	print("RESTORING STATE -", state)
	...
	pre_controls = new_controls.duplicate(true)
	apply_control_attributes()
